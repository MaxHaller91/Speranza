...
import { TRAITS, TRAIT_KEYS, fs,} from "../gameData.js";
    ...
    {pending.map(col => (
      <div key={col.id} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          <div style={{ fontSize: fs(16) }}>⭐</div>
          <div>
            <div style={{ color: "#f5a623", fontSize: fs(12), fontWeight: "bold", letterSpacing: 2 }}>
              {col.name} — LEVEL {col.level} REACHED
            </div>
            <div style={{ color: "#7a5a20", fontSize: fs(8), letterSpacing: 1, marginTop: 2 }}>
          ...
          {TRAIT_KEYS.filter(k => !col.traits.includes(k)).map(traitKey => {
            ...
            return (
              <button key={traitKey} onClick={() => onPickTrait(col.id, traitKey)} style={{
                ...
                <div style={{ color: trait.color, fontSize: fs(11), fontWeight: "bold", marginBottom: 3 }}>
                  {trait.icon} {trait.label}
                </div>
                <div style={{ color: "#5a5040", fontSize: fs(8), lineHeight: 1.4 }}>{trait.desc}</div>
              </button>
            );
          })}
        </div>
