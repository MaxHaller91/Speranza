...
export default function SidePanel({
  ...
  return (
    <div style={{ width: fs(205), display: "flex", flexDirection: "column", gap: 8, flexShrink: 0 }}>

      {/* Colony Log */}
      {journalOpen && (
        <div style={{ background: "#050710", border: "1px solid #0d1520", borderRadius: 8, padding: 10 }}>
          <div style={{ color: "#2a4a6a", fontSize: fs(9), letterSpacing: 2, marginBottom: 6 }}>COLONY LOG</div>
          <div style={{ fontSize: fs(8), lineHeight: 1.9, maxHeight: 160, overflowY: "auto" }}>
            {log.map((entry, i) => (
      ...
      {effectsOpen && (
        <div style={{ background: "#0b0c12", border: "1px solid #2a2414", borderRadius: 8, padding: 10 }}>
          <div style={{ color: "#d4a843", fontSize: fs(9), letterSpacing: 2, marginBottom: 7 }}>🧪 COLONY EFFECTS</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: fs(8) }}>
            ...
            <div style={{ color: "#556677", fontSize: fs(8), lineHeight: 1.4 }}>{surfaceCondition.desc}</div>
          ...
          <div style={{ marginTop: 8, borderTop: "1px solid #1a1d2a", paddingTop: 6 }}>
            <div style={{ color: "#7a6a4a", fontSize: fs(8), letterSpacing: 1, marginBottom: 4 }}>RECENT DILEMMA OUTCOMES</div>
            {recentDilemmaOutcomes.length === 0 ? (
              <div style={{ color: "#445", fontSize: fs(8), fontStyle: "italic" }}>No recent outcomes.</div>
            ) : (
              <div style={{ maxHeight: 96, overflowY: "auto", display: "flex", flexDirection: "column", gap: 5 }}>
                {recentDilemmaOutcomes.slice(0, 4).map((outcome) => (
                  <div key={outcome.id} style={{ borderBottom: "1px solid #121522", paddingBottom: 3 }}>
                    <div style={{ color: "#8a9aaa", fontSize: fs(8) }}>[{tickToDayHour(outcome.tick)}]</div>
                    <div style={{ color: "#b8c8d8", fontSize: fs(8) }}>{outcome.choice}</div>
                    <div style={{ color: "#556677", fontSize: fs(8) }}>{outcome.summary}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ background: "#080b14", border: "1px solid #1a2a3a", borderRadius: 8, padding: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <div style={{ color: "#c8d0d8", fontSize: fs(11), fontWeight: "bold", letterSpacing: 1 }}>{selCol.name}</div>
                {(selCol.level ?? 0) > 0 && (
                  <div style={{ color: selCol.level >= 3 ? "#ffd700" : "#aaaaaa", fontSize: fs(8), background: selCol.level >= 3 ? "#1a1400" : "#111", border: `1px solid ${selCol.level >= 3 ? "#ffd70044" : "#33333344"}`, borderRadius: 3, padding: "1px 4px" }}>LVL {selCol.level}</div>
                )}
              </div>
              <button onClick={onCloseColonist} style={{ background: "none", border: "1px solid #1a2535", borderRadius: 3, color: "#445", padding: "1px 5px", cursor: "pointer", fontSize: fs(10) }}>✕</button>
            </div>
            <div style={{ color: statusColor, fontSize: fs(8), letterSpacing: 1, marginBottom: 8 }}>
              {STATUS_LABEL[selCol.status]}{selCol.status === "injured" && selCol.injuryTicksLeft > 0 ? ` — ${selCol.injuryTicksLeft}t to recover` : ""}
            </div>
            <div style={{ marginBottom: 8 }}>
              <div style={{ color: "#2a4a6a", fontSize: fs(8), letterSpacing: 1, marginBottom: 3 }}>EXPERIENCE — LVL {selCol.level ?? 0}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <div style={{ color: "#2a4a6a", fontSize: fs(8), fontFamily: "monospace" }}>{xpInLevel}/20</div>
              </div>
            </div>
            {selCol.traits?.length > 0 && (
              <div style={{ marginBottom: 8 }}>
                <div style={{ color: "#2a4a6a", fontSize: fs(8), letterSpacing: 1, marginBottom: 4 }}>TRAITS</div>
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                  {selCol.traits.map(t => (
                    <div key={t} title={TRAITS[t]?.desc} style={{ fontSize: fs(8), background: "#0a0a14", border: `1px solid ${TRAITS[t]?.color ?? "#333"}44`, borderRadius: 3, padding: "1px 5px", color: TRAITS[t]?.color ?? "#888" }}>
            ...
            {selCol.quirk && (
              <div style={{ marginBottom: 8 }}>
                <div style={{ color: "#2a4a6a", fontSize: fs(8), letterSpacing: 1, marginBottom: 4 }}>QUIRK</div>
                <div style={{ background: "#0a0a18", border: "1px solid #2a2a5a", borderRadius: 4, padding: "5px 8px" }}>
                  <div style={{ color: "#9988cc", fontSize: fs(9), marginBottom: 2 }}>{selCol.quirk.icon} {selCol.quirk.label}</div>
                  <div style={{ color: "#556677", fontSize: fs(8), lineHeight: 1.5 }}>{selCol.quirk.desc}</div>
                </div>
              </div>
            )}
            <div style={{ marginBottom: 8 }}>
              <div style={{ color: "#2a4a6a", fontSize: fs(8), letterSpacing: 1, marginBottom: 4 }}>BACKGROUND</div>
              <div style={{ color: "#445566", fontSize: fs(8), lineHeight: 1.6, fontStyle: "italic" }}>{selCol.backstory || "No record."}</div>
            </div>
            <div>
              <div style={{ color: "#2a4a6a", fontSize: fs(8), letterSpacing: 1, marginBottom: 4 }}>SERVICE RECORD</div>
              {[
                ...
                <div key={label} style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                  <span style={{ color: "#445", fontSize: fs(8) }}>{label}</span>
                  <span style={{ color: "#7a9aaa", fontSize: fs(8), fontFamily: "monospace" }}>{val}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {selCell?.type && !buildMenu && (
        <div style={{ background: "#080b14", border: `1px solid ${ROOM_TYPES[selCell.type].border}44`, borderRadius: 8, padding: 10 }}>
          <div style={{ color: ROOM_TYPES[selCell.type].color, fontSize: fs(11), letterSpacing: 1, marginBottom: 3 }}>
            {ROOM_TYPES[selCell.type].icon} {ROOM_TYPES[selCell.type].label.toUpperCase()}
          </div>
          <div style={{ color: "#445", fontSize: fs(8), marginBottom: selCell.damaged ? 6 : 8 }}>{ROOM_TYPES[selCell.type].desc}</div>

          {selCell.damaged && (
            <div style={{ background: "#1a0000", border: "1px solid #ff4444", borderRadius: 4, padding: "5px 7px", marginBottom: 8 }}>
              <div style={{ color: "#ff4444", fontSize: fs(8), fontWeight: "bold" }}>⚠ STRUCTURAL DAMAGE</div>
              <div style={{ color: "#884444", fontSize: fs(8), marginTop: 2 }}>Room offline — not producing. Repair to restore.</div>
            </div>
          )}

          {ROOM_TYPES[selCell.type].cap > 0 && (
            <>
              <div style={{ color: "#667", fontSize: fs(9), marginBottom: 4 }}>WORKERS: {selCell.workers}/{ROOM_TYPES[selCell.type].cap}</div>
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => onAssign(selected.r, selected.c, -1)} style={{ flex: 1, background: "#1a0a0a", border: "1px solid #5a2a2a", borderRadius: 4, color: "#c44", padding: 4, cursor: "pointer", fontSize: fs(14) }}>−</button>
                <button onClick={() => onAssign(selected.r, selected.c,  1)} style={{ flex: 1, background: "#0a1a0a", border: "1px solid #2a5a2a", borderRadius: 4, color: "#4c4", padding: 4, cursor: "pointer", fontSize: fs(14) }}>+</button>
              </div>
              <div style={{ marginTop: 6, fontSize: fs(8) }}>
          ...
          {selCell.type === "armory" && (
            <div style={{ marginTop: 10 }}>
              {!armoryArmed ? (
                <div style={{ fontSize: fs(9), color: "#5a3a3a", border: "1px solid #3a1a1a", borderRadius: 4, padding: "6px 8px", textAlign: "center" }}>
                  Assign 1 armorer above to unlock expeditions
                </div>
              ) : (<>
                <div style={{ marginBottom: 8 }}>
                  <div style={{ color: "#884444", fontSize: fs(9), letterSpacing: 1, marginBottom: 4 }}>DURATION</div>
                  <div style={{ display: "flex", gap: 4 }}>
                    {[20, 40, 60, 80].map(d => (
                      <button key={d} onClick={() => onSetExpedDuration(d)} style={{
                        ...
                        padding: "3px 0", cursor: "pointer", fontSize: fs(8), fontFamily: "monospace",
                      }}>{d}t</button>
                    ))}
                  </div>
                  <div style={{ fontSize: fs(8), color: "#443333", marginTop: 3 }}>~{Math.floor(expedDuration / 8)} scav · ~{Math.floor(expedDuration / 6)} strike rolls</div>
                </div>
                  ...
                  return (
                    <div key={exp.id} style={{ background: "#0d0008", border: "1px solid #ff444422", borderRadius: 6, padding: 7, marginBottom: 6 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }}>
                        <div style={{ color: def.color, fontSize: fs(9), fontWeight: "bold" }}>{def.icon} {def.label}</div>
                        <div style={{ color: "#5a3a3a", fontSize: fs(8), fontFamily: "monospace" }}>{exp.ticksLeft}t</div>
                      </div>
                      <div style={{ fontSize: fs(8), color: "#665555", marginBottom: 4 }}>👤 {names}</div>
                      ...
                      {lastEvents.map((evt, i) => <div key={i} style={{ fontSize: fs(8), color: "#664444", lineHeight: 1.5, fontFamily: "monospace" }}>{evt}</div>)}
                      {lootStr && <div style={{ marginTop: 3, fontSize: fs(8), color: "#556633" }}>🎒 {lootStr}</div>}
                    </div>
                  );
                })}
                ...
                {expeditions.length < 2 ? (
                  <div>
                    {/* ── Destination ── */}
                    <div style={{ color: "#884444", fontSize: fs(9), letterSpacing: 1, marginBottom: 5 }}>DESTINATION</div>
                      ...
                      {SURFACE_LOCATIONS.map(loc => {
                        ...
                        return (
                          <button key={loc.id} onClick={() => onSetExpedLocation(loc.id)} title={loc.flavor} style={{
                            ...
                            <span style={{ fontSize: fs(10) }}>{loc.icon}</span>
                            <span style={{ flex: 1, fontSize: fs(8), color: sel ? loc.color : "#66707a", letterSpacing: 0.5 }}>{loc.label}</span>
                            <span style={{ fontSize: fs(6.5), color: RISK_COLOR[loc.risk] ?? "#667" }}>{loc.risk}</span>
                          </button>
                        );
                      })}
                    </div>
                    {selectedLocation && (
                      <div style={{ fontSize: fs(8), color: "#4a5560", lineHeight: 1.5, marginBottom: 8, fontStyle: "italic", borderLeft: `2px solid ${selectedLocation.color}55`, paddingLeft: 6 }}>
                    ...
                    {/* ── Crew ── */}
                    <div style={{ color: "#884444", fontSize: fs(9), letterSpacing: 1, marginBottom: 5 }}>
                      CREW {expedCrewIds.length > 0 && <span style={{ color: "#5a7a5a" }}>· {expedCrewIds.length} selected</span>}
                    </div>
                    {eligibleCrew.length === 0 ? (
                      <div style={{ fontSize: fs(8), color: "#5a3a3a", marginBottom: 8 }}>No eligible colonists — free someone from a post.</div>
                    ) : (
                        ...
                        {eligibleCrew.map(c => {
                          ...
                          return (
                            <button key={c.id}
                              ...
                              style={{
                                fontSize: fs(8), letterSpacing: 0.5, fontFamily: "monospace",
                    ...
                    {/* ── Launch ── */}
                    <div style={{ color: "#884444", fontSize: fs(9), letterSpacing: 1, marginBottom: 6 }}>LAUNCH</div>
                    {Object.entries(EXPEDITION_TYPES).map(([key, def]) => {
                      ...
                      return (
                        <button key={key} onClick={() => onLaunchExpedition(key)} disabled={!canSend} style={{
                          ...
                          <div style={{ fontSize: fs(11), color: canSend ? def.color : "#333" }}>{def.icon} {def.label}</div>
                          <div style={{ fontSize: fs(8), color: canSend ? "#556" : "#222", marginTop: 2, lineHeight: 1.4 }}>{def.desc}</div>
                          <div style={{ fontSize: fs(8), color: canSend ? "#883333" : "#222", marginTop: 3 }}>
                            {canSend
                  ...
                  <div style={{ fontSize: fs(8), color: "#5a3a3a", border: "1px solid #3a1a1a", borderRadius: 4, padding: "6px 8px", textAlign: "center" }}>Max 2 expeditions active</div>
                )}
              </>)}
            </div>
          )}
          ...
          {selCell.type === "hospital" && (() => {
            ...
            return (
              <div style={{ marginTop: 10 }}>
                <div style={{ color: "#ff6b9d", fontSize: fs(9), letterSpacing: 1, marginBottom: 6 }}>PATIENTS — {injured.length} in recovery</div>
                {nurseCount === 0 && injured.length > 0 && (
                  <div style={{ fontSize: fs(8), color: "#7a4a4a", background: "#1a0008", border: "1px solid #5a2030", borderRadius: 4, padding: "5px 7px", marginBottom: 6 }}>⚠ No nurses assigned — healing at 25% speed</div>
                )}
                {nurseCount > 0 && <div style={{ fontSize: fs(8), color: "#556", marginBottom: 6 }}>{nurseCount} nurse{nurseCount > 1 ? "s" : ""} · treating up to {capacity} patients</div>}
                {injured.length === 0 ? (
                  <div style={{ fontSize: fs(8), color: "#334", fontStyle: "italic" }}>No patients currently.</div>
                ) : injured.map(col => (
                  <div key={col.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#100008", border: "1px solid #ff6b9d33", borderRadius: 4, padding: "4px 7px", marginBottom: 4 }}>
                    <div style={{ color: "#c8d0d8", fontSize: fs(9) }}>{col.name}</div>
                    <div style={{ color: "#ff6b9d", fontSize: fs(8), fontFamily: "monospace" }}>⚕ {col.injuryTicksLeft ?? 0}t</div>
                  </div>
                ))}
              </div>
            );
          ...
          {selCell.type === "researchLab" && (
            <div style={{ marginTop: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div style={{ color: "#00e5ff", fontSize: fs(9), letterSpacing: 1 }}>T2 TECHNOLOGIES</div>
                <div style={{ color: "#00e5ff", fontSize: fs(9), fontFamily: "monospace" }}>{Math.floor(res.rp)} RP</div>
              </div>
              {Object.entries(T2_TECHS).map(([key, tech]) => {
                ...
                return (
                  <div key={key} style={{ marginBottom: 6, background: unlocked ? "#001a10" : "#0a0c14", border: `1px solid ${unlocked ? "#00e5ff" : canAfford ? "#00e5ff44" : "#1a2030"}`, borderRadius: 5, padding: "6px 8px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }}>
                      <div style={{ color: unlocked ? "#00e5ff" : canAfford ? "#aaa" : "#445", fontSize: fs(10) }}>{tech.icon} {tech.label}</div>
                      {unlocked ? (
                        <div style={{ color: "#00e5ff", fontSize: fs(8) }}>✓ DONE</div>
                      ) : (
                        <button onClick={() => onUnlockTech(key)} disabled={!canAfford} style={{
                          ...
                          padding: "2px 6px", cursor: canAfford ? "pointer" : "not-allowed", fontSize: fs(8),
                        }}>{tech.cost} RP</button>
                      )}
                    </div>
                    <div style={{ color: "#334", fontSize: fs(8), lineHeight: 1.4 }}>{tech.desc}</div>
                  </div>
                );
              })}
            </div>
          ...
          {selCell.type === "shelter" && (() => {
            ...
            return (
              <div style={{ marginTop: 10 }}>
                <div style={{ color: "#7ecfb4", fontSize: fs(9), letterSpacing: 1, marginBottom: 6 }}>SHELTER STATUS — {sheltered.length} sheltering</div>
                {alarmOn ? (<>
                  <div style={{ background: "#001a0f", border: "1px solid #7ecfb433", borderRadius: 4, padding: "6px 8px", marginBottom: 8 }}>
                    {sheltered.map(col => <div key={col.id} style={{ color: "#7ecfb4", fontSize: fs(9), paddingBottom: 2 }}>🏠 {col.name}</div>)}
                  </div>
                  <button onClick={onBackToWork} style={{ width: "100%", background: "#001a0f", border: "1px solid #7ecfb4", borderRadius: 4, color: "#7ecfb4", padding: "6px 8px", cursor: "pointer", fontSize: fs(9), letterSpacing: 1 }}>🏠 BACK TO WORK</button>
                </>) : (<>
                  <div style={{ fontSize: fs(8), color: "#446655", marginBottom: 6 }}>Sounds the alarm and pulls all idle/working colonists into shelter. Production stops but they cannot be targeted by Arc strikes.</div>
                  <button onClick={onSoundAlarm} style={{ width: "100%", background: "#1a0808", border: "1px solid #ff4444", borderRadius: 4, color: "#ff6666", padding: "6px 8px", cursor: "pointer", fontSize: fs(9), letterSpacing: 1 }}>🚨 SOUND ALARM</button>
                </>)}
              </div>
            );
          })()}
          ...
          {selCell.type === "sentryPost" && (
            <div style={{ marginTop: 10 }}>
              <div style={{ color: "#e8d44d", fontSize: fs(9), letterSpacing: 1, marginBottom: 4 }}>SENTRY STATUS</div>
              <div style={{ background: "#0d0f00", border: "1px solid #e8d44d33", borderRadius: 4, padding: "6px 8px" }}>
                {selCell.workers === 0 ? (
                  <div style={{ color: "#5a5020", fontSize: fs(8) }}>No sentries assigned.</div>
                ) : (<>
                  <div style={{ color: "#c8d0d8", fontSize: fs(9) }}>🪖 {selCell.workers} sentry{selCell.workers > 1 ? "ies" : ""} active</div>
                  <div style={{ color: "#e8d44d", fontSize: fs(8), marginTop: 3 }}>-{Math.min(60, selCell.workers * 18)}% heat gain</div>
                  <div style={{ color: "#5a5020", fontSize: fs(8), marginTop: 2 }}>Sentries are exposed during raids.</div>
                </>)}
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 5, marginTop: 10, flexWrap: "wrap" }}>
            <button onClick={onCloseRoom} style={{ flex: 1, background: "none", border: "1px solid #1e2a3a", borderRadius: 4, color: "#445", padding: 4, cursor: "pointer", fontSize: fs(9) }}>CLOSE</button>
            {selCell.damaged && (
      ...
      {!selected && artifacts.length > 0 && (
        <div style={{ background: "#0b0a06", border: "1px solid #2a2410", borderRadius: 8, padding: 10, marginBottom: 8 }}>
          <div style={{ color: "#8a7a4a", fontSize: fs(9), letterSpacing: 2, marginBottom: 8, borderBottom: "1px solid #2a2410", paddingBottom: 6 }}>
          ...
          <div style={{ maxHeight: 170, overflowY: "auto" }}>
            {artifacts.slice().reverse().map((a, i) => (
              <div key={a.id ?? i} style={{ marginBottom: 9, paddingBottom: 7, borderBottom: i < artifacts.length - 1 ? "1px solid #1c1a10" : "none" }}>
                <div style={{ color: "#c8b070", fontSize: fs(8), letterSpacing: 0.5, marginBottom: 3 }}>
                  {a.colonist} · DAY {a.day}
                </div>
                <div style={{ color: "#6a6250", fontSize: fs(8), fontStyle: "italic", lineHeight: 1.5 }}>{a.text}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {!selected && (
        <div style={{ background: "#080b14", border: "1px solid #1a2030", borderRadius: 8, padding: 10 }}>
          {memorial.length > 0 ? (<>
            <div style={{ color: "#7a6a4a", fontSize: fs(9), letterSpacing: 2, marginBottom: 8, borderBottom: "1px solid #2a1a0a", paddingBottom: 6 }}>🕯 MEMORIAL</div>
              ...
              {memorial.map((entry, i) => (
                <div key={entry.id ?? i} style={{ marginBottom: 10, paddingBottom: 8, borderBottom: i < memorial.length - 1 ? "1px solid #1a1a2a" : "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                    <span style={{ color: "#c8d0d8", fontSize: fs(9), fontWeight: "bold" }}>{entry.name}</span>
                    <span style={{ color: "#555", fontSize: fs(8) }}>LVL {entry.level}</span>
                  </div>
                  <div style={{ color: "#c87a30", fontSize: fs(8), letterSpacing: 0.5, marginBottom: 3 }}>
                    {CAUSE_LABEL[entry.cause] ?? "Left the colony"}
                    {" — "}DAY {entry.day} · {entry.hour}
                  </div>
                  <div style={{ color: "#445", fontSize: fs(8), fontStyle: "italic", lineHeight: 1.5 }}>{entry.epitaph}</div>
                </div>
              ))}
            </div>
          </>) : (
            <div style={{ color: "#2a3a4a", fontSize: fs(9), textAlign: "center", padding: "10px 0", letterSpacing: 1 }}>Select a cell to build or manage a room</div>
          )}
        </div>
      )}
    </div>
