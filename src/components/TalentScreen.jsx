...
import { TALENTS, TALENT_ORDER } from "../gameData.js";
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }}>
          <div style={{ color: "#4ab3f4", fontSize: 14, letterSpacing: 3 }}>✦ TALENTS</div>
          <div style={{ color: "#7ed321", fontSize: 13, fontFamily: "monospace" }}>
            {resolve} <span style={{ color: "#3a5a2a", fontSize: 9, letterSpacing: 1 }}>RESOLVE</span>
          </div>
        </div>
          ...
          {TALENT_ORDER.map(key => {
            ...
            return (
              <div key={key} style={{
                ...
                <div style={{ fontSize: 15, width: 20, textAlign: "center", opacity: have ? 1 : 0.55 }}>{t.icon}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ color: have ? "#7ed321" : "#8fa4b8", fontSize: 10, fontWeight: "bold", letterSpacing: 1 }}>
                    {t.label}
                  </div>
                  <div style={{ color: "#5a6a7a", fontSize: 9, marginTop: 2, lineHeight: 1.45 }}>{t.desc}</div>
                </div>
                {have ? (
                  <div style={{ color: "#7ed321", fontSize: 9, letterSpacing: 1, padding: "0 6px" }}>OWNED</div>
                ) : (
                  <button
